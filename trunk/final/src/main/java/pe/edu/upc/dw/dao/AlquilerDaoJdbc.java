...
public class AlquilerDaoJdbc {
    ...
    public boolean registrarAlquiler(Alquiler oAlquiler) {
        ...
        try {
            ...
            st = connection.createStatement();
            sQuery = "Insert into Dew.Alquiler(Id_Alquiler, Fecha, Hora_Inicio, Placa, Id_Local, Id_Estado) values ('" + nIdAlquiler +
                    "', '" + oAlquiler.getFecha() + "','" + oAlquiler.getHora_inicio() + "','" + oAlquiler.getPlaca() +
                    "','" + oAlquiler.getLocal().getId() + "', '01')";
    ...
    public Alquiler buscarAlquiler(Alquiler oAlquiler) {
        ...
        try {
            ...
            String sQuery = "Select a.Id_Alquiler, a.Fecha, a.Hora_Inicio, a.Placa, a.Id_Local, l.nombre from Dew.Alquiler a " +
                    ...
                    " Where a.Placa = '" + oAlquiler.getPlaca() + "' and Id_Estado ='01'";
            rs = st.executeQuery(sQuery);
            if (rs.next()) {
...
//            if (oAlquiler.getTarjeta() == null) {
//                nTarjeta = 0;
//            }
//            else{
//            nTarjeta = oAlquiler.getTarjeta().getId();
//            }

            String sQuery = "Update Dew.Alquiler Set Hora_Fin = '" + oAlquiler.getHora_fin() +
                    "', Importe = " + oAlquiler.getImporte() + ", Id_Tarjeta='" + oAlquiler.getTarjeta().getId() +
                    "', Id_Estado='" + oAlquiler.getEstado() + "' Where Id_Alquiler = '" + oAlquiler.getId() + "'";
            nOk = st.executeUpdate(sQuery);
            if (nOk >= 0) {
                bOk = true;
            }
