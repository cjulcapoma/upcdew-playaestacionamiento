	...
	var vcform = document.forms[0].name;
        ...
        switch (vcform)
        {
            ...
            case "cust_reg":
                ...
                var vbabon = document.forms[0].TipoCliente[0].checked;

                if (vcnombre==''){
                    alert('Por favor, ingrese el nombre del cliente');
                    return false;
                }
                //Solo solicitar los siguintes datos si es del tipo abonado
                if(vbabon){
                    if (vcdireccion==''){
                    ...
                    if (vcmail==''){
                        ...
                        if( !(/\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)/.test(vcmail)) && !(/^[A-Za-z][A-Za-z0-9_.]*@[A-Za-z0-9_]+\.[A-Za-z0-9_.]+[A-za-z]$/.test(vcmail)) ) {
            ...
            case "card_load":
                ...
                if (!IsNum(vcmonto)){
                    alert('Por favor, ingrese un valor num\xe9rico o un valor mayor a cero');
                    return false;
                }
            break;
        }
...
function IsNum( numstr ) {	//  || (!/^([0-9])*[.]?[0-9]*$/.test(vcmonto))
//alert('IsNum antes del if 0');
if (numstr+"" == "undefined" || numstr+"" == "null" || numstr+"" == ""  ) return false;
var isValid = true;
var decCount = 0;
numstr += "";
//alert('IsNum antes del for');
 for (i = 0; i < numstr.length; i++) {
     ...
     if (!((numstr.charAt(i) >= "0") && (numstr.charAt(i) <= "9") || (numstr.charAt(i) == ".")) ) {
         ...
         isValid = false;
         break;
     } else if ((numstr.charAt(i) == "." && numstr.length == 1) ||  (numstr.charAt(i) == "." && decCount > 1)) {
         isValid = false;
         break;
     }
     }
